...
// netlify/functions/ical.ts
var ical_exports = {};
__export(ical_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(ical_exports);
...
var supabase = (0, import_supabase_js.createClient)(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    // Disabilita auto-login da URL hash
    autoRefreshToken: true,
    persistSession: true
  }
});
...
// services/ical.ts
var generateHubberICalFeed = async (hubberId, token, listingId) => {
  ...
  const { data: bookings, error } = await supabase.from("bookings").select(`
    ...
    `).eq("hubber_id", hubberId).eq("listing_id", listingId).in("status", ["confirmed", "accepted", "active", "completed"]).order("start_date", { ascending: true });
  if (error) {
    console.error("Errore recupero prenotazioni per iCal:", error);
    throw new Error("Errore generazione calendario");
  }
  const events = (bookings || []).map((booking) => {
    const renterName = booking.renter ? `${booking.renter.first_name || ""} ${(booking.renter.last_name || "").charAt(0)}.`.trim() : "Renter";
    ...
    return {
      ...
      dtend: formatDateForICal(booking.end_date, true),
      // +1 giorno per end date
      summary: `\u{1F511} ${listingTitle} - ${renterName}`,
      description: [
      ...
      lastModified: formatDateTimeForICal((/* @__PURE__ */ new Date()).toISOString())
    };
  });
...
var generateICalString = (events, options = {}) => {
  const {
    calendarName = "RentHubber Calendar",
    calendarDescription = "Prenotazioni RentHubber"
  } = options;
  const now = formatDateTimeForICal((/* @__PURE__ */ new Date()).toISOString());
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
...
var formatDateForICal = (isoDate, addOneDay = false) => {
...
var formatDateTimeForICal = (isoDate) => {
  const date = new Date(isoDate);
...
var escapeICalText = (text) => {
  return text.replace(/\\/g, "\\\\").replace(/;/g, "\\;").replace(/,/g, "\\,").replace(/\n/g, "\\n").replace(/\r/g, "");
};
var foldICalLines = (content) => {
...
var handler = async (event, context) => {
  ...
  try {
    ...
    if (!userId || !token || !listingId) {
      return {
        statusCode: 400,
        body: "Missing userId, token, or listingId"
        // 🆕 Aggiorna messaggio
      };
    }
    const icalContent = await generateHubberICalFeed(userId, token, listingId);
    return {
      statusCode: 200,
      headers: {
