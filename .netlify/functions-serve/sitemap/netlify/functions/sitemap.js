...
var handler = async (event, context) => {
  try {
    const { data: listings, error } = await supabase.from("listings").select("id, created_at, status").eq("status", "published").is("deleted_at", null).order("created_at", { ascending: false });
    if (error) {
