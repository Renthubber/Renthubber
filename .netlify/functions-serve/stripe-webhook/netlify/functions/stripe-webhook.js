...
var handler = async (event, context) => {
  ...
  const stripe = new stripe_esm_node_default(STRIPE_SECRET_KEY, {
  ...
  console.log("\u{1F3A3} Webhook received:", stripeEvent.type);
  console.log("\u{1F3A3} Webhook received:", stripeEvent.type);
  try {
    switch (stripeEvent.type) {
      case "payment_intent.succeeded":
        await handlePaymentIntentSucceeded(stripeEvent.data.object, SUPABASE_URL, SUPABASE_ANON_KEY);
        break;
      case "payment_intent.payment_failed":
        await handlePaymentIntentFailed(stripeEvent.data.object);
        break;
      case "account.updated":
        await handleAccountUpdated(stripeEvent.data.object, SUPABASE_URL, SUPABASE_ANON_KEY);
        break;
      default:
...
async function handlePaymentIntentSucceeded(paymentIntent, SUPABASE_URL, SUPABASE_ANON_KEY) {
  ...
  console.log("\u{1F389} Payment processing completed");
}
...
async function handleAccountUpdated(account, SUPABASE_URL, SUPABASE_ANON_KEY) {
  console.log("\u{1F504} Account updated:", account.id);
