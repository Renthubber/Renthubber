...
// netlify/functions/process-payout-auto.ts
var process_payout_auto_exports = {};
__export(process_payout_auto_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(process_payout_auto_exports);
...
var AUTO_PAYOUT_CONFIG = {
  enabled: process.env.AUTO_PAYOUT_ENABLED === "true",
  // Toggle generale
  maxAmountAuto: parseFloat(process.env.AUTO_PAYOUT_MAX_AMOUNT || "200"),
  // Max €200 automatico
  minBalance: parseFloat(process.env.AUTO_PAYOUT_MIN_BALANCE || "50"),
  // Min €50 per richiedere
  requireVerifiedAccount: true,
  // Account deve essere completamente verificato
  checkOpenDisputes: true,
  // Verifica dispute aperte
  minAccountAgeDays: 30,
  // Account hubber deve avere almeno 30 giorni
  minCompletedBookings: 5
  // Almeno 5 booking completati
};
var handler = async (event, context) => {
  ...
  try {
    ...
    console.log("\u{1F916} Processing automatic payout:", payoutRequestId);
    ...
    console.log("\u{1F4CA} Checking eligibility:", {
    ...
    const accountAgeDays = Math.floor((Date.now() - hubberSince.getTime()) / (1e3 * 60 * 60 * 24));
    ...
    console.log("\u{1F389} Automatic payout processed successfully");
    return {
      statusCode: 200,
      headers,
      body: JSON.stringify({
...
async function updatePayoutStatus(payoutRequestId, status, reason) {
  await fetch(
    `${SUPABASE_URL}/rest/v1/payout_requests?id=eq.${payoutRequestId}`,
    {
      method: "PATCH",
...
//# sourceMappingURL=process-payout-auto.js.map
